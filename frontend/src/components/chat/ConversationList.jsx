...
const ConversationList = ({
  ...
  if (loading) {
    return (
      <div className="h-full flex flex-col bg-glass-card">
        <div className="p-4 border-b border-glass-border space-y-3">
          <div className="h-6 w-32 bg-glass-card rounded animate-pulse" />
          <div className="h-9 bg-glass-card rounded-xl animate-pulse" />
        </div>
        <div className="flex-1 p-3 space-y-2">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-16 bg-glass-card rounded-xl animate-pulse"
            />
          ))}
        </div>
      </div>
    ...
    <div className="h-full flex flex-col bg-glass-card">
      ...
      <div className="p-4 border-b border-glass-border space-y-3 shrink-0">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-text-title tracking-tight">
            Discussions
          </h2>
            ...
            <button
              ...
              className={`p-1.5 rounded-lg border text-xs flex items-center gap-1 transition ${
                notifPermission === "granted"
                  ...
                  ? "bg-bg-darker border-glass-border text-text-muted cursor-not-allowed"
                  : "bg-sky-500/10 border-sky-500/30 text-sky-500 hover:bg-sky-500/20 cursor-pointer animate-pulse"
              }`}
              title={
                notifPermission === "granted"
        ...
        {notifPermission === "default" && (
          <div
            onClick={onRequestNotifPermission}
            className="p-2 bg-sky-500/10 border border-sky-500/20 rounded-lg flex items-center justify-between cursor-pointer hover:bg-sky-500/20 transition"
          >
            <div className="flex items-center gap-2">
              <Bell size={14} className="text-sky-500 shrink-0" />
              <span className="text-[11px] text-text-title font-medium leading-tight">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-bg-darker border border-glass-border rounded-xl text-xs text-text-title placeholder:text-text-muted focus:outline-none focus:border-sky-500/50 transition-all duration-150"
          />
        </div>
      </div>
