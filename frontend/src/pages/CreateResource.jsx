...
import { ArrowLeft, PlusCircle, Trash2, Edit2, X, Plus, AlertCircle, Info, FileText, Link as LinkIcon, BookOpen, Tag } from "lucide-react";
...
const CreateResource = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFiles.length + files.length > 5) {
      triggerToast("Maximum of 5 documents allowed.");
      return;
    }
  ...
  const handleSave = async (targetStatus) => {
    ...
    try {
      ...
      if (res && res.success) {
        triggerToast(`Resource successfully saved as ${targetStatus}`);
        setTimeout(() => {
          navigate("/");
        }, 1000);
      }
    } catch (err) {
      setFormError(err.response?.data?.message || `Failed to create resource as ${targetStatus}.`);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-6 text-left max-w-4xl mx-auto relative pb-12">
      ...
      <div className="border-b border-glass-border/40 pb-5">
        <h1 className="text-2xl font-extrabold text-text-title flex items-center gap-2">
          <PlusCircle className="text-accent-purple" size={24} /> Create Resource Draft
        </h1>
        <p className="text-xs text-text-muted font-semibold mt-1">
          Draft a new knowledge resource with scripts, attachments, and URLs.
        </p>
      </div>

      <form onSubmit={(e) => e.preventDefault()} className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        <div className="lg:col-span-8 space-y-6">
          <SpotlightCard className="p-6 text-xs space-y-4" glowColor="rgba(168, 85, 247, 0.05)">
            ...
            <div className="space-y-1">
              ...
              <input
                type="text"
                placeholder="e.g. Production-grade Docker Compose Setup for MERN Stack"
                value={title}
            ...
            <div className="space-y-1">
              ...
              <textarea
                placeholder="Describe what configurations, guidelines, scripts, or links this resource bundle provides..."
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                rows={6}
                className="w-full form-input rounded-xl p-3 bg-bg-dark text-text-title border-glass-border focus:border-accent-purple/50 focus:outline-none leading-relaxed"
                required
              />
            </div>
              ...
              <div className="space-y-1">
                ...
                {loadingCategories ? (
                  ...
                  <select
                    ...
                    className="w-full form-input rounded-xl p-3 bg-bg-dark text-text-title border-glass-border focus:border-accent-purple/50 cursor-pointer"
                    required
                  >
                    {categories.map((cat) => (
          ...
          <SpotlightCard className="p-6 text-xs space-y-4" glowColor="rgba(168, 85, 247, 0.05)">
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  <Plus size={14} className="text-accent-purple" /> Choose Files
                </label>
              </div>

              {selectedFiles.length === 0 ? (
                <p className="text-center py-4 text-text-muted italic">No documents chosen yet.</p>
              ) : (
                <div className="space-y-2.5">
                  <h4 className="font-bold text-text-muted uppercase text-[9px] tracking-wider">Selected files:</h4>
                  <div className="space-y-2.5">
        ...
        {/* Right tags and actions sidebar column */}
        <div className="lg:col-span-4 space-y-6">
          ...
          <SpotlightCard className="p-6 text-xs space-y-4" glowColor="rgba(168, 85, 247, 0.05)">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-grow form-input rounded-xl p-2.5 bg-bg-dark text-text-title border-glass-border focus:border-accent-purple/50 focus:outline-none"
              />
            ...
            {topics.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2 pt-1.5">
                {topics.map((t, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1 bg-accent-purple/10 text-accent-purple border border-accent-purple/20 px-2.5 py-1 rounded-lg font-bold"
                  >
                    <Tag size={10} /> {t}
          ...
          <SpotlightCard className="p-6 text-xs space-y-4" glowColor="rgba(168, 85, 247, 0.05)">
            ...
            <p className="text-text-muted leading-relaxed text-[10px]">
              Upon creation, your new resource will be placed in <strong>draft status</strong>. You can review, add attachments, and publish it when ready from your owned dashboard.
            </p>
